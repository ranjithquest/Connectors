    ...
    return (
      ...
      <div className="features-grid">
        {cards.map((f, idx) => {
          ...
          const previewDeckUrl = f.previewUrl ? f.previewUrl.replace('/connectors', '/deck') : undefined;
          const previewWalkthroughUrl = f.previewUrl ? `${f.previewUrl}?tour=1` : undefined;
          const decks = (f.decks && f.decks.length > 0) ? f.decks : (previewDeckUrl ? [{ label: 'Deck', url: previewDeckUrl }] : []);
          const walkthroughs = (f.walkthroughs && f.walkthroughs.length > 0) ? f.walkthroughs : (previewWalkthroughUrl ? [{ label: 'Walkthrough', url: previewWalkthroughUrl }] : []);
          return (
            <div key={f.branch}
...
export default function AboutPage() {
  ...
  function loadBranches() {
    ...
    fetchJson<{ features?: Feature[] }>(url)
  ...
  useEffect(() => {
    ...
    const isHosted = typeof window !== 'undefined' && !window.location.hostname.includes('localhost');
    if (!isHosted) {
      fetchJson<{ branch?: string; ownerSlug?: string }>('/api/current-branch/')
        .then((data) => {
          setCurrentBranch(data?.branch ?? '');
          setOwnerSlug(data?.ownerSlug ?? '');
        })
        .catch(() => {});
      fetchJson<{ branches?: LocalBranch[] }>('/api/local-branches/')
        .then((data) => setLocalBranches(data?.branches ?? []))
        .catch(() => {});
    }
  }, []);
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const data = await readJsonResponse<{ branch?: string; error?: string }>(res);
      if (!data) {
        setCreateError('Unexpected response from branch creation endpoint');
        setCreating(false);
        return;
      }
      ...
      fetchJson<{ branch?: string; ownerSlug?: string }>('/api/current-branch/')
        .then((nextData) => {
          setCurrentBranch(nextData?.branch ?? '');
          setOwnerSlug(nextData?.ownerSlug ?? '');
        })
        .catch(() => {});
      fetchJson<{ branches?: LocalBranch[] }>('/api/local-branches/')
        .then((nextData) => setLocalBranches(nextData?.branches ?? []))
        .catch(() => {});
